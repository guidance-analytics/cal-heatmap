const dayTemplate = (DateHelper, { domain, verticalOrientation, }) => {
    const ROWS_COUNT = 7;
    const ALLOWED_DOMAIN_TYPE = ['year', 'month', 'week'];
    return {
        name: 'day',
        ...
        columnsCount: (ts) => {
            switch (domain.type) {
                case 'month':
                    return Math.ceil(domain.dynamicDimension && !verticalOrientation ?
                        DateHelper.getMonthWeekNumber(DateHelper.date(ts).endOf('month')) :
                        6);
                case 'year':
                    return Math.ceil(domain.dynamicDimension ?
        ...
        mapping: (startTimestamp, endTimestamp) => {
            let weekNumber = 0;
            let x = -1;
            return DateHelper.intervals('day', startTimestamp, DateHelper.date(endTimestamp)).map((ts) => {
