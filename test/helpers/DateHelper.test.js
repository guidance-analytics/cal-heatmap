...
describe('DateHelper', () => {
    ...
    beforeEach(() => __awaiter(void 0, void 0, void 0, function* () {
        ...
        yield dateHelper.setup(options);
    }));
    describe('setup()', () => {
        describe('when using the default locale', () => {
            it('instantiate dayjs with the default locale', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        describe('when using locale FR', () => {
            describe('on browser env', () => {
                it('instantiate dayjs with the FR locale', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    describe('dayjs()', () => {
        describe('is locale aware', () => {
            it('returns monday as first week day on FR locale', () => __awaiter(void 0, void 0, void 0, function* () {
                options.init({ date: { locale: 'fr', timezone: 'utc' } });
                yield dateHelper.setup(options);
                expect(dateHelper.date().startOf('week').format('dddd')).toEqual('lundi');
            }));
            it('returns sunday as first week day on EN locale', () => __awaiter(void 0, void 0, void 0, function* () {
                options.init({ date: { locale: 'en', timezone: 'utc' } });
                yield dateHelper.setup(options);
                expect(dateHelper.date().startOf('week').format('dddd')).toEqual('Sunday');
            }));
        });
    });
        ...
        for (const [locale, value] of Object.entries(weekData)) {
            ...
            value.forEach((key, index) => {
                key.forEach((monthDate) => {
                    it(`assigns ${monthDate} to the week ${index + 1}`, () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    describe('getWeeksCountInMonth()', () => {
        ...
        it('returns the number of weeks', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    describe('format()', () => {
        it('returns the value returned by the given function', () => {
            expect(dateHelper.format(+date, (d, v) => `${new Date(d)}-${v}`, 'hello')).toEqual(`${date}-hello`);
        });
                ...
                describe(`on ${interval} interval`, () => {
                    describe('when date args is a Date object', () => {
                        it(`returns [date start of ${interval}]`, () => {
                            ...
                            expect(new Date(intervals[0])).toEqual(expectations[intervalKey][0]);
                        });
                    });
                    ...
                    describe('when range args is -1', () => {
                        it('returns [start of previous interval]', () => {
                            const intervals = dateHelper.intervals(interval, expectations[intervalKey][1], -1);
                            expect(toDates(intervals)).toEqual([
                                expectations[intervalKey][0],
                            ]);
                        });
                    ...
                    describe('when range args is a negative number < -1', () => {
                        it(`returns [${interval}](x range)`, () => {
                            //
                            const intervals = dateHelper.intervals(interval, expectations[intervalKey][3], -3);
                            expect(toDates(intervals)).toEqual(expectations[intervalKey].slice(0, 3));
                            expect(intervals.length).toEqual(3);
                        });
                    });
                    describe('when range args is a future date, ' +
                        //
                        '1 interval in the future', () => {
                        it(`returns [${interval}]`, () => {
                            const intervals = dateHelper.intervals(interval, date, expectations[intervalKey][0]);
                            expect(toDates(intervals)).toEqual([
                                expectations[intervalKey][0],
                            ]);
                        });
                    ...
                    describe('when range args is a future date, ' +
                        ...
                        it(`returns [${interval}](3)`, () => {
                            const intervals = dateHelper.intervals(interval, date, expectations[intervalKey][3]);
                            expect(toDates(intervals)).toEqual(expectations[intervalKey].slice(0, 3));
                        });
                    });
                });
