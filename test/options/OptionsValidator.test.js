...
describe('OptionsValidator', () => {
    ...
    validSubDomainTemplate.settings = new Map([
        ['day', { allowedDomainType: ['month'] }],
        ['x_day', { allowedDomainType: ['month'] }],
        ['month', { allowedDomainType: ['month'] }],
    ]);
    ...
    it('only accepts dataType from the whitelist', () => {
        expect(() => {
            validate(validSubDomainTemplate, Object.assign(Object.assign({}, options.options), { 
                // @ts-ignore
                data: { type: 'hello' } }));
        }).toThrow();
        expect(validate(validSubDomainTemplate, {
            domain: { type: 'month' },
            subDomain: { type: 'day' },
            data: { type: 'json' },
        })).toBe(true);
