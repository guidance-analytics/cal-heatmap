...
describe('DomainCollection', () => {
    ...
    describe('when filling with data', () => {
        let filledDomainCollection;
        beforeEach(() => {
            filledDomainCollection = new DomainCollection(dummyDateHelper, 'day', 1, 1);
    ...
    describe('extractTimestamp()', () => {
        ...
        it.each([
            {
                param: dd.extractTimestamp({ date: 1577836800000 }, 'date', (a) => a),
                title: 'a timestamp',
            },
            {
                param: dd.extractTimestamp({ date: '2020-01-01' }, 'date', (a) => a),
                title: 'a date without timezone',
            },
            {
                param: dd.extractTimestamp({ date: '2020-01-01T00:00:00.000Z' }, 'date', (a) => a),
                title: 'a date with UTC timezone',
            },
            {
                param: dd.extractTimestamp({ date: 'Wed Jan 01 2020 04:00:00 GMT+0400 (Gulf Standard Time)' }, 'date', (a) => a),
                title: 'a date with a custom timezone',
            },
            {
                param: dd.extractTimestamp({ date: '2020-01-01 04:00:00.000+04:00' }, 'date', (a) => a),
    ...
    describe('groupValues()', () => {
        ...
        describe('on string values', () => {
            it('returns the result from the function', () => {
                expect(d.groupValues(['Asia', 'Europe', 'America'], (n) => n[0])).toBe('Asia');
            });
            it('executes the specified group function name', () => {
                expect(d.groupValues(['Asia', 'Europe', 'America', null], 'count')).toBe(3);
            });
        });
    });
});
